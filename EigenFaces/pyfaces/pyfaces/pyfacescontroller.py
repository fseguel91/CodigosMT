...
class PyFaceController(object):
    ...
    def validateSelection(self,fileName,directoryName,numOfEigenfaces,thresholdVal):
        if fileName is '':
            ...
            print 'validateSelection()::ningun archivo seleccionado!!'
            #error=pyfacesgui.NoFileSelectError('no file selected')
            error=pyfacesgui.NoFileSelectError('ningun archivo seleccionado')
            self.updateResults(error)
        elif directoryName is '':
            #print 'validateSelection()::no directory selected!!'
            print 'validateSelection()::ningun directorio selecionado!!'
            #error=pyfacesgui.NoDirSelectError('no directory selected')
            error=pyfacesgui.NoDirSelectError('ningun directorio selecionado')
            self.updateResults(error)
        else:
    ...
    def processMatchResult(self,matchfile,mindist,numOfEigenfaces):
        if not matchfile:
            ...
            error=pyeigenfaces.NoMatchError('No hay resultados! intente con threshold mas alto')            
...
if __name__ == "__main__":
    ...
    root = Tk()
    root.wm_title("Reconocimientos de Rostros en Raspberry Pi")
